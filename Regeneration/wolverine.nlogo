extensions [mgr  gis table csv math mgr ]
...
interventions-own [
  ...
  intervention-active? ; is the intervention ongoing?
...
to setup
  ...
  (ifelse model-area = "Jarva/Stockholm" [
    ...
    error (word "Unrecognized model-area setting: \"" model-area "\"")
  ])
  ...
  ifelse intervention-file != "" and intervention-file != "NA" [
...
to-report gis-union-of-table [ a-table ]
  ...
  if is-number? union [
    error "BUG: union requested of empty table"
  ]
  report union
end
...
to find-junctions [road-type]
  if any? junctions with [junction-type = road-type] [
    error (word "Already created network for \"" road-type "\"")
  ]
...
to connect-road-network [road-type]
  ...
  if any? routes with [route-type = road-type] [
    error (word "Already created route for \"" road-type "\"")
  ]
...
to go
  ...
  ask households [
    if household-finance < 0 [
      set unhappiness-money lput ticks unhappiness-money
    ...
    if household-n-encounters > 0 and household-n-same-encounters / household-n-encounters < household-homophily [
      set unhappiness-heterophily lput ticks unhappiness-heterophily
    ...
    if length unhappiness-crime > 0 and ticks - last unhappiness-crime >= household-forgiveness-crime [
      set unhappiness-crime filter [ t -> ticks - t <= household-forgetting-crime ] unhappiness-crime
    ]
    if length unhappiness-money > 0 and ticks - last unhappiness-money >= household-forgiveness-money [
      set unhappiness-money filter [ t -> ticks - t <= household-forgetting-money ] unhappiness-money
    ]
    if length unhappiness-heterophily > 0 and ticks - last unhappiness-heterophily >= household-forgiveness-hetero [
      set unhappiness-heterophily filter [ t -> ticks - t <= household-forgetting-hetero ] unhappiness-heterophily
    ]
    if length unhappiness-racism > 0 and ticks - last unhappiness-racism >= household-forgiveness-racism [
  ...
  ask households [
    ...
    if consecutive-days-unhappy >= n-unhappy-move and not member? self move-waiting-list [
  ...
  if hh-file != "NA" and hh-file != "" and hh-file-write-frequency > 0 and ticks mod hh-file-write-frequency = 0 [
...
to households-init-variables
  ...
  set household-apartment one-of household-suitable-apartments true

  if household-apartment = nobody [
    error (word "Could not find an apartment for household " who " with SCB type " hh-scb-type ", SCB dwelling type " hh-dw-type ", floor space "
  ...
  set unhappiness-racism []
  set unhappiness-crime []
  set unhappiness-money []
  set unhappiness-heterophily []
...
to households-apply-to-move
  set move-waiting-list lput self move-waiting-list
  set n-move-requests n-move-requests + 1
end

to association-offer-houses
  repeat moves-per-tick [
    if length move-waiting-list > 0 [
      let request-hh first move-waiting-list

      ifelse find-apartment-for-hh? request-hh options-per-move [
        set move-waiting-list but-first move-waiting-list
      ] [
        set move-waiting-list lput request-hh (but-first move-waiting-list)
      ]
    ]
  ]
end
...
to-report find-apartment-for-hh? [ hh tries ]
  ...
  ask hh [
    let suitable-apartments household-suitable-apartments false
...
to-report household-suitable-apartments [ same-borough? ]
  ...
  if same-borough? [
    ...
    if count available = 0 [
      ...
      if count available = 0 [
        ; then compromise on tenancy

        set available borough-apartments
        let compromise-tenancy? true

        if count available = 0 [
          ; finally, compromise on borough
          set available household-suitable-apartments false
          ...
          if count available = 0 [
            ...
            if count available = 0 [
              error "There are no apartments that are not occupied or being renovated; reduce the number of households or increase the number of apartments per building"
            ]
...
to-report household-better-apartments
  ...
  if count selection = 0 [
    ...
    if count selection = 0 [
      ...
      if count selection = 0 [
        error ("No spare apartments to move a household into during renovation. Increase the number of apartments.")
      ]
...
to move-household [ an-apartment ]
  if [apartment-occupied?] of an-apartment [
    error "Attempt to move a household to an occupied apartment"
  ]
  ask household-apartment [
...
to households-create-network
  ...
  ask households with [household-employed?] [
    create-social-ties-with other households in-radius household-workplace-reach with [distance myself < household-workplace-reach]
  ]
...
to households-pay-rent-and-bills
  let rent-to-pay [apartment-rent] of household-apartment
  let bills-to-pay [apartment-rent] of household-apartment
  ; Pay rent and bills
...
to households-accumulate-encounters
  ...
  if (here-n-encounters > 0                                                  ; IF there's at least 1 other person here ...
    ...
    ask one-of households-here with [household-ethnicity != my-ethnicity] [  ; THEN pick someone to give some kind of racism-related unhappiness to
      set unhappiness-racism lput ticks unhappiness-racism
      if networked-unhappiness? [
...
;{households}
;{households receive "training" on enviromental matters and reduce their energy consumption}
to households-eco-training
  set household-habits-eco (household-habits-eco / 2)
end
...
to households-update-crime
  if household-criminal? and random-float 1 < p-crime [
    ...
    ask victim [
      set unhappiness-crime lput ticks unhappiness-crime
...
to-report unhappy-crime?
  report length unhappiness-crime >= household-tolerance-crime
end

to-report unhappy-money?
  report length unhappiness-money >= household-tolerance-money
end

to-report unhappy-heterophily?
  report length unhappiness-heterophily >= household-tolerance-hetero
end

to-report unhappy-racism?
  report length unhappiness-racism >= household-tolerance-racism
end
    ...
    create-interventions 1 [
      ...
      (foreach iv-header iv [ [ key value ] ->
        (ifelse key = "start" [
          ...
          error (word "Unrecognized column heading \"" key "\" in intervention file \"" file-name "\"")
        ])

        set intervention-buildings buildings with [ (burgh = "all" or burgh = [borough-name] of building-borough)
          and building-type = "A-apartment" and building-year <= built and n-dwellings >= min-size ]
      ])
    ]
  ]
end
...
to save-interventions [ file-name ]
  ...
  ask interventions [
    let burgh-list remove-duplicates ([[borough-name] of building-borough] of intervention-buildings)
    let built max ([building-year] of intervention-buildings)
    let min-size min ([n-dwellings] of intervention-buildings)
    ...
    file-print csv:to-string (list (list intervention-start intervention-duration (member? "crime" intervention-notification-information)
      ...
      intervention-household-eco-habits-change intervention-meetings (ifelse-value length burgh-list = 1 [ first burgh-list ] [ "all" ])
      built min-size))
  ]
  file-close
end
...
to random-intervention
  create-interventions 1 [
    ...
    let iv-buildings buildings with [ building-borough = burgh and building-type = "A-apartment" and count building-residents > 0 ]
...
to start-intervention
  ...
  ask intervention-apartments [
    set apartment-renovation-status "ongoing"
...
to step-intervention
  (ifelse (intervention-stage = "start") [
    ...
    ifelse intervention-household-eco-habits-change > 0 or length intervention-building-progress = 0 [
      set intervention-building-progress []
    ...
    error (word "Unrecognized intervention-stage: \"" intervention-stage "\"")
  ])
end
...
;
; Find a suitable apartment for the household while renovation of their apartment is ongoing
...
to-report intervention-find-temporary-apartment
  ...
  if count candidate-apartments = 0 [
    error "Not found a temporary apartment for a household to move in to"
  ]
...
to apartments-init-variables [a-building]
  ...
  ;;; Renovation ;;;
  set apartment-renovated? false
  set apartment-renovation-time 999
...
to apartments-update-variables
  if (apartment-renovation-status = "on-going" and apartment-renovation-time > 0) [
  ...
  set apartment-energy-consumption (24 - daylight-hours) + ((apartment-size * kwh-per-m2 * apartment-usage-efficiency) / (30 * 24)) ; avg. consumption: 12 kWh/month per square meter / 30 days Data source: seer SMRD 23.20.2020 see "Rent-and-energy-determination-per-apartment.xlsx"
  set apartment-energy-cost (floor ((apartment-energy-consumption * 100.1) )) / 100 ; avg. cost of energy: 1.1. SEK/month per kWh per day, see details in SMRD 23.20.2020

end

to buildings-update-variables
  let n n-dwellings
  let h count building-apartments with [apartment-occupied?]
  let r count building-apartments with [apartment-occupied? and [household-protesting?] of apartment-household]
...
to apartments-renovation-starts [renovation-time]
  ...
  set apartment-renovation-status "on-going" ; previously "not renovated"
end
...
to initialize-borough
  (ifelse borough-name = "Tensta" [
    ...
    error (word "Unrecognized borough-name: \"" borough-name "\"")
  ])
...
to env-read-weather-data
  ...
  if not file-exists? weather-file [ error (word "Weather file does not found. Please, check file names and directory.") ]
  ...
  set env-monthly-mean (map [ i -> mean i ] env-monthly-list)
...
to initialize-scb [ scb-dir ]
  ...
  set scb-household-tenancy read-scb-csv (word scb-dir "/household-tenancy.csv")
end
...
to-report read-scb-csv [ filename ]
  ...
  if length first-heading = 0 [
    error (word "No usable heading name in first column of first row of \"" filename "\" (\"" (first csv-headings) "\")")
  ]
...
to-report scb-sample-building-year
  ...
  carefully [
    ...
    error (word "Could not get a year range from \"" const-year-range "\" -- minimum \"" (substring const-year-range 0 4) "\"; maximum \"" (substring const-year-range 5 9) "\": " error-message)
  ]
    ...
    if scb-region-id = scb-municipality and scb-hh-type-id = hh-type and scb-dwelling-type = dw-type [
      ...
      ifelse floor-space = -1 [
        ...
        if floor-space != scb-floor-space [
          error (word "Duplicate entry for region \"" scb-region-id "\", household type \"" scb-hh-type-id "\", dwelling type \"" scb-dwelling-type "\" in household-type-floor-space file (" floor-space ", " scb-floor-space ")")
        ]
      ]
    ]
  ]
  if floor-space = -1 or floor-space = ".." [
    error (word "No data for region \"" scb-municipality "\", household type \"" hh-type "\", dwelling type \"" dw-type "\" in household-type-floor-space file")
  ]
  report floor-space
end
...
to-report weighted-sample-index [ weight-list ]
  if length weight-list = 0 [
    error "weighted-sample-index called with empty weight-list"
  ]
...
to-report scb-gender-from-hh-type [ hh-type ]
  report ifelse-value (hh-type = "LEK" or hh-type = "LEKP" or hh-type = "LEKB") [ "F" ] [
    ifelse-value (hh-type = "LEM" or hh-type = "LEMP" or hh-type = "LEMB") [ "M" ] [ "C" ]
  ]
end
...
to-report scb-n-wage-from-hh-type [ hh-type ]
  report ifelse-value (hh-type = "LEK" or hh-type = "LEM" or hh-type = "LEKB" or hh-type = "LEMB") [ 1 ] [
    ifelse-value (hh-type = "LS" or hh-type = "LS1" or hh-type = "LS2" or hh-type = "LS3" or hh-type = "LO" or hh-type = "LOMB") [ 2 ] [ 0 ]
...
to-report scb-hh-size-from-hh-type [ hh-type ]
  report scb-n-wage-from-hh-type hh-type + ifelse-value (hh-type = "LEKP" or hh-type = "LEMP" or hh-type = "LS1" or hh-type = "LO") [ 1 ] [
    ifelse-value (hh-type = "LEKB" or hh-type = "LEMB" or hh-type = "LS2" ) [ 2 ] [
      ifelse-value (hh-type = "LOMB" or hh-type = "LS3") [ 3 ] [ 0 ]
    ]
  ]
  ; N.B. This basically assumes 'other' households have one non-wage earning member if there are no children, and three non-wage-earning members if there are
  ; Single parent households are assumed to have two children
end
...
to-report scb-building-type-from-dwelling-type [ dw-type ]
  report ifelse-value (dw-type = "SMAG" or dw-type = "SMBO" or dw-type = "SMHY0") [ "one- or two-dwelling buildings" ] [
    ifelse-value (dw-type = "FBBO" or dw-type = "FBHY0") [ "multi-dwelling-buildings" ] [
      ifelse-value (dw-type = "SPBO") [ "special housing" ] [ "other buildings" ]
    ]
  ]
end
...
to-report scb-tenancy-type-from-dwelling-type [ dw-type ]
  report ifelse-value (dw-type = "SMAG") [ "owner-occupied dwellings" ] [
    ifelse-value (dw-type = "SMBO" or dw-type = "FBBO") [ "tenant-owned dwellings" ] [
      ifelse-value (dw-type = "SMHY0" or dw-type = "FBHY0") [ "rented dwellings" ] [ "unknown" ]
    ]
  ]
end
...
"ethnicity > 3" 5.0 0 -10899396 true "" "histogram [count social-tie-neighbors] of households with [household-ethnicity > 3]"
...
"" ""
PENS
"default" 100.0 0 -16777216 true "" "histogram [household-income] of households"
...
"" ""
PENS
"waiting" 1.0 0 -16777216 true "" "plot length move-waiting-list"

...
globals [
  ...
  energy-types; This will be "electric" "gas" "heat" "gas-tank" "peat" "oil-tank" (and "new-technology" in one scenario)
...
households-own [
  ...
  stored-energy; this is for peat, oil-tank and gas-tank systems.
  energy-used-today
  energy-used-today-for-heating
...
to setup
  ...
  (ifelse
    ...
    extent = "Aberdeen" or extent = "Torry"
    [
      load-an-energy-provider-from-a-file "AHP"
      load-an-energy-provider-from-a-file "DEAL"
    ]
    extent = "Barra" or extent = "Uist" [
      load-an-energy-provider-from-a-file "peat"
    ]
  )
  ...
  (ifelse extent = "Aberdeen" or extent = "Torry" [
  ...
  debug "setup: ...finished laying pipe"
...
to gis
  ...
  (ifelse
    ...
    extent = "Uist" [
      set abm-map-file "uist/uist"
    ]
    extent = "Barra" [
      set abm-map-file "barra/Barra"
    ]
  )
  ...
  [
    ...
    (ifelse
      ...
      extent = "Uist" [
        set streets-pathname (word gis-dir "/uist/Uists_gis_osm_ROADS_free_1.shp")
      ]
      extent = "Barra" [
        set streets-pathname (word gis-dir "/barra/Barra_osm_ROADS_free_1.shp")
      ]
    )
...
to events
  ...
  if ticks > tick-new-technology [
    set energy-types ["electric" "gas" "heat" "new-technology" "gas-tank" "oil-tank" "peat" "solar" "heat-pump" "air-pump"]
  ]
...
to daily
  ...
  debug "daily: fuel supplies"
  ask households with [ heating-system = "peat" or heating-system = "oil-tank" or heating-system = "gas-tank" ] [
    if energy-used-today-for-heating < stored-energy [
      ; We need to order some more fuel or dig some more peat
      ; So how do we want to do this?
      ; The options are a full-tank, or some level off a full tank
      ; I am going to assume until told otherwise the supplies are always available.
      ; Note this applies to gas-tank/oil-tank/peat - the pet has no price
      let fuel-bought 0
      ifelse random-float 1.0 < probability-of-full-tank [
        set fuel-bought energy-for-a-full-tank - stored-energy
      ]
      [
        set fuel-bought random-float (energy-for-a-full-tank - stored-energy)
      ]
      set stored-energy stored-energy + fuel-bought

      if heating-system != "peat" [
        let some-energy-provider nobody
        let some-tariff ""
        let lowest-cost 999999999
        ask energy-providers with [ member? heating-system table:values energy-type and member? "ad-hoc" table:values frequency ]  [
          foreach table:keys frequency [ a-tariff ->
            if (table:get frequency a-tariff = "ad-hoc"
             and table:get energy-type a-tariff = heating-system
             and table:get retail-unit-cost a-tariff < lowest-cost) [
              set lowest-cost table:get retail-unit-cost a-tariff
              set some-tariff a-tariff
              set some-energy-provider self
            ]
          ]
        ]
        pay-energy-provider some-energy-provider some-tariff heating-system
      ]
    ]
  ]
...
to calculate-energy-consumption
  set energy-used-today 0
  set energy-used-today-for-heating 0
  ...
  if heating-system != "" [
    ...
    set energy-used-today-for-heating (((optimal - PET) *
    ...
    set energy-used-today-for-heating (24 - hours-away-from-household /
      ( count my-in-members-of * 24 )) * energy-used-today-for-heating
    set spend [table:get retail-unit-cost tariff] of this-energy-provider * energy-used-today-for-heating
    if balance - spend - money-spent < 0  and not any? out-associate-neighbors [
      set history-of-temperature lput current-temperature history-of-temperature
      table:put units-of-energy-used heating-system 0
      if current-temperature < ideal-temperature [
    ...
    set energy-used-today energy-used-today + energy-used-today-for-heating
    set money-spent money-spent + spend
    table:put units-of-energy-used heating-system table:get units-of-energy-used heating-system + energy-used-today-for-heating
    set current-temperature ideal-temperature
    ask this-energy-provider [
      let costs [table:get wholesale-unit-cost tariff] of this-energy-provider * energy-used-today-for-heating
      set profitability profitability - costs - overhead
      set daily-income replace-item 0 daily-income (item 0 daily-income + spend)
      set daily-outgoings replace-item 0 daily-outgoings (item 0 daily-outgoings - costs - overhead)
    ]
      ...
      ask trusters [
        ...
        ask who-to-consult [
          ...
          [
            ...
            if (([heating-system] of some-household = "heat" and (extent = "Aberdeen" or extent = "Torry")) or
              ([heating-system] of some-household = "new-technology" and (extent = "Aberdeen" or extent = "Torry") and ticks > tick-new-technology) or
      ...
      ask trusters [
        ...
        (ifelse
          heuristic-type = "innovator" [
            if ((some-decision = "install" and (extent = "Aberdeen" or extent = "Torry") and item 0 some-state = "heat")
              or (some-decision = "install" and (extent = "Aberdeen" or extent = "Torry") and item 0 some-state = "new-technology" and ticks > tick-new-technology)
          ...
          heuristic-type = "laggard" [
            if ((some-decision = "install" and (extent = "Aberdeen" or extent = "Torry") and item 0 some-state = "heat")
              ...
              ask my-building [
                (ifelse
                  table:get has? "heat" and (extent = "Aberdeen" or extent = "Torry") [
                    set for for + 1
                  ]
...
; {household | business } install

to install [ some-heating-system ]
  debug  (word "install: INSTALLING..." some-heating-system)

  if some-heating-system = heating-system [
    debug  (word "install: TRYING TO REINSTALL " some-heating-system)
    stop
  ]

  if heating-system = "insulation" [
    (ifelse
      energy-rating = "G" [
        set energy-rating "F"
      ]
  ...
  if heating-system = "peat" and is-business? self [
    debug  (word "install: business is trying to install peat as a source of heating")
  ]
  ...
  if [table:get installation-cost this-payment-name] of this-energy-provider > balance [
    debug (word "install: CANNOT AFFORD " some-heating-system)
    stop
  ]
  set contract-expires 0
  if fixed-contract-length > -1 [
...
to-report random-affordability
  ...
  let maintenance []
  ask energy-providers [
    set install-cost (sentence install-cost table:values installation-cost)
  ...
  let random-install (random-normal mean install-cost (max install-cost - min install-cost) / 4)
  if random-install < 0 [
...
; {observer} structures-for-barra

to structures-for-barra


  debug  (word date-and-time ":structures-for-barra: setting up houses and businesses...")
;  let abm-map-file "barra/Barra"
;  let abm-map-data gis-load-dataset (word gis-dir "/" abm-map-file ".shp")
;  gis:set-world-envelope gis:envelope-of abm-map-data
  if not display-gis? [
    err "structures-for-barra: gis-display? set to false"
  ]

  let abm-map-build-data gis-load-dataset   (word gis-dir "/" "barra" "/" "Barra_gis_osm_buildings_a_free_1.shp")
  ...
  foreach gis:feature-list-of abm-map-build-data [ vector-feature ->
    set record-count record-count + 1
    if record-count >= nof-buildings and limit-buildings? [
      debug (word ":structures-for-barra: ...ended setting up " count buildings " buildings.")
      stop
    ]
    if record-count mod 100 = 0 [
      debug (word "structures-for-barrra: processed " record-count " vector features.")
    ]
    let vector-feature-coordinates gis:location-of gis:centroid-of vector-feature
    ...
    if not empty? vector-feature-coordinates [
      ...
      ask this-patch [
        ...
        [distance myself]
        ifelse target-road = nobody [
          debug  (word "structures-for-barra: building "
            " name: " some-name
            " street-name " some-street-name
            " postcode " some-postcode
            " have has info missing.")
        ...
        ifelse (
          member? "Auto" some-name or
          member? "Centre" some-name or
          member? "Hotel" some-name or
          member? "Hostel" some-name or
          member? "Airport" some-name or
          member? "Co-op" some-name or
          some-type = "industrial" or
          some-type = "retail" or
          some-type = "garage" or
          some-type = "greenhouse" or
          some-type = "church" or
          some-type = "university" or
          some-type = "hospital" or
          some-type = "school") [
  ...
  debug (word ":structures-for-barrra: ...ended setting up " count buildings " buildings.")
...
to structures-for-timisoara
  ...
  debug  (word date-and-time ":structures-for-timisoara: setting up houses and businesses...")
  if display-gis? [
    ...
    ask roads with [
      ...
      (ifelse
        ...
        member? "offsetting-costs" street-voting  [
          let district-heating-provider ""
          (ifelse
            extent = "Timisoara" [ set district-heating-provider one-of energy-providers with [name = "Colterm"] ]
            extent = "Torry" [ set district-heating-provider one-of energy-providers with [name = "AHP"] ]
            extent = "Aberdeen" [ set district-heating-provider one-of energy-providers with [name = "AHP"] ]
          )
...
to update-energy-provider-with [ data-path ]
  ...
  while [ not file-at-end? ] [
    ...
    if item 1 data != "electric" and
      ...
      item 1 data != "heat-pump" and
      item 1 data != "gas-tank" and
      item 1 data != "oil-tank" and
      item 1 data != "peat" and
      item 1 data != "new-technology" [
      error (word "Field 2, energy type must be one " energy-types " for record " data)
    ]
...
extent
extent
"Torry" "Aberdeen" "Timisoara" "Uist" "Barra"
4
